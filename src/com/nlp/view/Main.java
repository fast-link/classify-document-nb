...
public class Main extends javax.swing.JFrame {
    ...
    private void classify(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_classify
     Classifier classifier = new Classifier("naiveBayes.txt", 5000);//su dung 3000 tu khoa
        String storyDirectory = System.getProperty("user.dir") + "\\testdata";
 
        String temp = "" ;
        String message = "Please wait...";
 
        result.setText(message);
        File[] files = FileUtils.getAllFileInFolder(storyDirectory);
        ...
        for (File file : files) {
            ...
            switch (detectResult.getKey()) {
                case Classifier.BOOK_TYPE_SWORD_HERO: {
                    String s  = "Truyện kiếm hiệp: " + "độ chắc chắn: " +  detectResult.getValue() + " %" + "\n";
                    temp = temp + s ;
                   // JOptionPane.showMessageDialog(null, String.format("Truyện kiếm hiệp: %s\nđộ chắc chắn: %.2f phần trăm", file.getName(), detectResult.getValue()));
                    break;
                }
                case Classifier.BOOK_TYPE_OTHER: {
                    String s  = "Không phải truyện kiếm hiệp " +  "độ chắc chắn: " +  detectResult.getValue() + " %" + "\n";;
                    temp = temp + s ;
                   // JOptionPane.showMessageDialog(null, String.format("Truyện khác: %s\nđộ chắc chắn: %.2f phần trăm", file.getName(), detectResult.getValue()));
                    break;
                }
            }
        }
